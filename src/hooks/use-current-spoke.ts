...
export const useCurrentSpoke = () => {
  ...
  const currentSpokeId = siteConfig.customFields?.currentSpokeId as string | undefined;
  if (currentSpokeId) {
  ...
  const rest = pathname.startsWith(baseUrl) ? pathname.slice(baseUrl.length) : pathname;
